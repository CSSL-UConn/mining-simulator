...
int main(int argc, const char *argv[]) {
    ...
    //#########################################################################################
    //idea of simulation: 2 miners, only an honest, and a stubborn lead trail fork miner. Run many games, with the
    //size of the two changing. Plot the expected profit vs. actual profit. 
    GAMEINFO("#####\nRunning Stubborn Lead Trail Fork Mining Simulation\n#####" << std::endl);
    std::ofstream plot;
    ...
    plot << "Gamma, Stubborn Lead Trail Fork Profit, Stubborn Lead Trail Fork Hash Rate, Honest Profit" << std::endl;
    //start running games

    for(double gammaVal = 0.005; gammaVal < 1.01; gammaVal+=0.005) {

    for(double hashVal = 0.005; hashVal < .51; hashVal+=0.005) {
        ...
        auto forkStrat1 = createStubbornLeadTrailForkStrategy(NOISE_IN_TRANSACTIONS, 1);


        MinerParameters stubbornLeadTrailForkMinerParams = {0, std::to_string(0), attackerPower, NETWORK_DELAY, COST_PER_SEC_TO_MINE};
        MinerParameters defaultinerParams = {2, std::to_string(1), honestPower, NETWORK_DELAY, COST_PER_SEC_TO_MINE};

        
        miners.push_back(std::make_unique<Miner>(stubbornLeadTrailForkMinerParams, *forkStrat1));
        ...
        GAMEINFO("Gamma" << "Fraction earned by stubborn lead trail fork:" << fractionOfProfits << " with " << attackerPower << " fraction of hash power" << "Fraction by honest: " << honestFractionOfProfits << std::endl);
