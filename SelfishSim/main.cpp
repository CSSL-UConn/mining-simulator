...
int main(int argc, const char *argv[]) {
    ...
    // Arg species the output fileame
    if (argc < 2) {
        std::cerr << std::endl;
        return 1;
    }
    ...
    GAMEINFO("#####\nRunning Classic Selfish Mining Simulation\n#####" << std::endl);
    std::ofstream plot;
    ...
    plot << "Gamma, Classic Selfish Profit, Classic Selfish Hash Rate, Honest Hash Rate" << std::endl;
    //start running games
    ...
    for(double hashVal = 0.005; hashVal < .51; hashVal+=.005) {
        ...
        GAMEINFO("Gamma" << "Fraction earned by classic selfish miner:" << fractionOfProfits << " with " << selfishPower << " fraction of hash power" << "Fraction by honest: " << honestFractionOfProfits << std::endl);
