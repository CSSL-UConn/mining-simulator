...
int main(int argc, const char *argv[]) {
    ...
    plot << "RationalVal, Gamma, Selfish Profit 1, Selfish Profit 2,  Individual Miner Hash Rate, Rational Profit,  Honest Profit" << std::endl;
    //start running games

    for (double rationalVal = 0.25; rationalVal < .51; rationalVal += 0.25 )
    ...
    for(double hashVal = 0.1; hashVal < 1-rationalVal; hashVal+=.01) {
        ...
        auto rationalProfits = valuePercentage(minerResults[2].totalProfit, result.moneyInLongestChain);
        auto honestFractionOfProfits = valuePercentage(minerResults[3].totalProfit, result.moneyInLongestChain);
        GAMEINFO("RationalVal" << "Gamma" << "Fraction earned by selfish miner:" << fractionOfProfits << " with " << selfishPower1 << " fraction of hash power" << "Fraction by second miner:" << fractionOfProfits2 << "Fraction by Rational" << rationalProfits <<  "Fraction by honest: " << honestFractionOfProfits << std::endl);
        plot << rationalVal << ", " << gammaVal << ", " << fractionOfProfits<< ", " << fractionOfProfits2 << ", " << selfishPower1 << ", " << rationalProfits << ", " << honestFractionOfProfits << std::endl;
        }
    }
    }  
