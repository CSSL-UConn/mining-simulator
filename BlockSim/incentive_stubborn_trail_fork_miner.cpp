//
//  incentive_stubborn_lead_trail_fork_miner.cpp
//  BlockSim


#include "incentive_stubborn_trail_fork_miner.hpp"
#include "block.hpp"
...
std::unique_ptr<Strategy> createIncentiveStubbornTrailForkStrategy(bool noiseInTransactions, Value trailCutoff, double incentiveFraction, Value forksToIncentivize, bool whaleEnabled, double whaleProb, double whaleMultiplier) {
    auto valueFunc = std::bind(defaultValueInMinedChild, _1, _2, noiseInTransactions, whaleEnabled, whaleProb, whaleMultiplier);
    
    auto bindedStubbornTrailForkBlockToMineOn = std::bind(incentiveStubbornTrailForkBlockToMineOn, std::placeholders::_1, std::placeholders::_2, trailCutoff);

    return std::make_unique<Strategy>("incentive-stubbornTrailFork", bindedStubbornTrailForkBlockToMineOn, valueFunc, 
    std::make_unique<IncentiveStubbornTrailForkPublishingStyle>(incentiveFraction, forksToIncentivize));
}

Block &incentiveStubbornTrailForkBlockToMineOn(const Miner &me, const Blockchain &chain, Value trailCutoff) {
...
std::vector<std::unique_ptr<Block>> IncentiveStubbornTrailForkPublishingStyle::publishBlocks(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) {
...
BlockHeight IncentiveStubbornTrailForkPublishingStyle::getPrivateHeadHeight(std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    return unpublishedBlocks.back()->height;
}

IncentiveStubbornTrailForkPublishingStyle::IncentiveStubbornTrailForkPublishingStyle(double incentive_, Value forksToIncentivize_) : PublishingStrategy(), incentiveFraction(incentive_), forksToIncentivize(forksToIncentivize_) {}

BlockHeight IncentiveStubbornTrailForkPublishingStyle::heightToPublish(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    assert(!unpublishedBlocks.empty());
