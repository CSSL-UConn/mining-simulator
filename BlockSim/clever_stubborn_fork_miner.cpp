//
//  clever_stubborn_fork_miner.cpp
//  BlockSim
//
...
std::unique_ptr<Strategy> createCleverStubbornForkStrategy(bool noiseInTransactions, Value cutoff) {
    ...
    return std::make_unique<Strategy>("clever-stubborn_fork", stubbornForkBlockToMineOn, valueFunc, std::make_unique<CleverStubbornForkPublishingStyle>(cutoff));
}

CleverStubbornForkPublishingStyle::CleverStubbornForkPublishingStyle(Value cutoff_) : StubbornForkPublishingStyle(), cutoff(cutoff_) {}

BlockHeight CleverStubbornForkPublishingStyle::heightToPublish(const Blockchain &blockchain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    ...
    if(unpublishedBlocks.back()->height == blockchain.getMaxHeightPub() + BlockHeight(1) &&
       ...
       unpublishedBlocks.size() == 1) {
        ...
        return StubbornForkPublishingStyle::heightToPublish(blockchain, me, unpublishedBlocks);
    }
}
