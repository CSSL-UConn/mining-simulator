...
#include "incentive_stubborn_lead_trail_fork_miner.hpp"
#include "block.hpp"
...
std::unique_ptr<Strategy> createIncentiveStubbornLeadTrailForkStrategy(bool noiseInTransactions, Value trailCutoff, double incentiveFraction, Value forksToIncentivize) {
    ...
    auto bindedStubbornLeadTrailForkBlockToMineOn = std::bind(incentiveStubbornLeadTrailForkBlockToMineOn, std::placeholders::_1, std::placeholders::_2, trailCutoff);

    return std::make_unique<Strategy>("incentive-stubbornLeadTrailFork", bindedStubbornLeadTrailForkBlockToMineOn, valueFunc, 
    std::make_unique<IncentiveStubbornLeadTrailForkPublishingStyle>(incentiveFraction, forksToIncentivize));
}

Block &incentiveStubbornLeadTrailForkBlockToMineOn(const Miner &me, const Blockchain &chain, Value trailCutoff) {
...
std::vector<std::unique_ptr<Block>> IncentiveStubbornLeadTrailForkPublishingStyle::publishBlocks(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) {
...
BlockHeight IncentiveStubbornLeadTrailForkPublishingStyle::getPrivateHeadHeight(std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    return unpublishedBlocks.back()->height;
}

IncentiveStubbornLeadTrailForkPublishingStyle::IncentiveStubbornLeadTrailForkPublishingStyle(double incentive_, Value forksToIncentivize_) : PublishingStrategy(), incentiveFraction(incentive_), forksToIncentivize(forksToIncentivize_) {}

BlockHeight IncentiveStubbornLeadTrailForkPublishingStyle::heightToPublish(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    assert(!unpublishedBlocks.empty());
