//
//  incentive_stubborn_trail_miner.cpp
//  BlockSim


#include "incentive_stubborn_trail_miner.hpp"
#include "block.hpp"
...
std::unique_ptr<Strategy> createIncentiveStubbornTrailStrategy(bool noiseInTransactions, Value trailCutoff, double incentiveFraction, Value forksToIncentivize, bool whaleEnabled, double whaleProb, double whaleMultiplier) {
    auto valueFunc = std::bind(defaultValueInMinedChild, _1, _2, noiseInTransactions, whaleEnabled, whaleProb, whaleMultiplier);
    
    auto bindedStubbornTrailBlockToMineOn = std::bind(incentiveStubbornTrailBlockToMineOn, std::placeholders::_1, std::placeholders::_2, trailCutoff);

    return std::make_unique<Strategy>("incentive-stubbornTrail", bindedStubbornTrailBlockToMineOn, valueFunc, 
    std::make_unique<IncentiveStubbornTrailPublishingStyle>(incentiveFraction, forksToIncentivize));
}

Block &incentiveStubbornTrailBlockToMineOn(const Miner &me, const Blockchain &chain, Value trailCutoff) {
...
std::vector<std::unique_ptr<Block>> IncentiveStubbornTrailPublishingStyle::publishBlocks(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) {
...
BlockHeight IncentiveStubbornTrailPublishingStyle::getPrivateHeadHeight(std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    return unpublishedBlocks.back()->height;
}

IncentiveStubbornTrailPublishingStyle::IncentiveStubbornTrailPublishingStyle(double incentive_, Value forksToIncentivize_) : PublishingStrategy(), incentiveFraction(incentive_), forksToIncentivize(forksToIncentivize_) {}

BlockHeight IncentiveStubbornTrailPublishingStyle::heightToPublish(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    assert(!unpublishedBlocks.empty());
