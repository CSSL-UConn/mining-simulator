...
std::unique_ptr<Strategy> createStubbornLeadTrailStrategy(bool noiseInTransactions, Value trailCutoff, bool whaleEnabled, double whaleProb, double whaleMultiplier) {
    auto valueFunc = std::bind(defaultValueInMinedChild, _1, _2, noiseInTransactions, whaleEnabled, whaleProb, whaleMultiplier);

   auto bindedStubbornLeadTrailBlockToMineOn = std::bind(stubbornLeadTrailBlockToMineOn, std::placeholders::_1, std::placeholders::_2, trailCutoff);

    return std::make_unique<Strategy>("stubbornLeadTrail", bindedStubbornLeadTrailBlockToMineOn, valueFunc, std::make_unique<StubbornLeadTrailPublishingStyle>());
}

Block &stubbornLeadTrailBlockToMineOn(const Miner &me, const Blockchain &chain, Value trailCutoff) {
...
std::vector<std::unique_ptr<Block>> StubbornLeadTrailPublishingStyle::publishBlocks(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) {
...
BlockHeight StubbornLeadTrailPublishingStyle::getPrivateHeadHeight(std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    return unpublishedBlocks.back()->height;
}

BlockHeight StubbornLeadTrailPublishingStyle::heightToPublish(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
       assert(!unpublishedBlocks.empty());
...
bool forkCameFirst = true;
    std::vector<Block*> possiblities = chain.oldestBlocks(chain.getMaxHeightPub());
     ...
     if (forkCameFirst && privateHeight == publicHeight + 1 && chain.blocksOfHeight(publicHeight) > BlockCount(1) ) {
        heightToPublish = privateHeight; // publish after fork
    } 
    return heightToPublish; // height to publish should be either the public height or private height (exactly 1 more). 
}
