...
std::unique_ptr<Strategy> createStubbornTrailStrategy(bool noiseInTransactions, Value trailCutoff, bool whaleEnabled, double whaleProb, double whaleMultiplier) {
    auto valueFunc = std::bind(defaultValueInMinedChild, _1, _2, noiseInTransactions, whaleEnabled, whaleProb, whaleMultiplier);
    
    auto bindedStubbornTrailBlockToMineOn = std::bind(stubbornTrailBlockToMineOn, std::placeholders::_1, std::placeholders::_2, trailCutoff);

    return std::make_unique<Strategy>("stubbornTrail", bindedStubbornTrailBlockToMineOn, valueFunc, std::make_unique<StubbornTrailPublishingStyle>());
}

Block &stubbornTrailBlockToMineOn(const Miner &me, const Blockchain &chain, Value trailCutoff) {
...
std::vector<std::unique_ptr<Block>> StubbornTrailPublishingStyle::publishBlocks(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) {
...
BlockHeight StubbornTrailPublishingStyle::getPrivateHeadHeight(std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    return unpublishedBlocks.back()->height;
}

BlockHeight StubbornTrailPublishingStyle::heightToPublish(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    assert(!unpublishedBlocks.empty());
