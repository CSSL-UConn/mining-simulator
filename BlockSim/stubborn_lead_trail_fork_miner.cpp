//
//  stubborn_lead_trail_fork_miner.cpp
//  BlockSim
...
std::unique_ptr<Strategy> createStubbornLeadTrailForkStrategy(bool noiseInTransactions, Value trailCutoff) {
    ...
    return std::make_unique<Strategy>("stubbornLeadTrailFork", bindedStubbornLeadTrailForkBlockToMineOn, valueFunc, std::make_unique<StubbornLeadTrailForkPublishingStyle>());
}

Block &stubbornLeadTrailForkBlockToMineOn(const Miner &me, const Blockchain &chain, Value trailCutoff) {
...
std::vector<std::unique_ptr<Block>> StubbornLeadTrailForkPublishingStyle::publishBlocks(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) {
...
BlockHeight StubbornLeadTrailForkPublishingStyle::getPrivateHeadHeight(std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    return unpublishedBlocks.back()->height;
}

BlockHeight StubbornLeadTrailForkPublishingStyle::heightToPublish(const Blockchain &chain, const Miner &me, std::vector<std::unique_ptr<Block>> &unpublishedBlocks) const {
    assert(!unpublishedBlocks.empty());
