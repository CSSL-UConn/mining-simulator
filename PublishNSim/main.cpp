...
int main(int argc, const char *argv[]) {
    ...
    GAMEINFO("#####\nRunning Publish N Mining Simulation\n#####" << std::endl);
    std::ofstream plot;
    ...
    plot << "Gamma, Publish N Profit, Publish N Hash Rate, Honest Hash Rate" << std::endl;
    //start running games
    ...
    for(double hashVal = 0.005; hashVal < .51; hashVal+=.005) {
        ...
        auto selfishStrat = createPublishNStrategy(NOISE_IN_TRANSACTIONS, 3);
        ...
        GAMEINFO("Gamma" << "Fraction earned by publish N:" << fractionOfProfits << " with " << selfishPower << " fraction of hash power" << "Fraction by honest: " << honestFractionOfProfits << std::endl);
