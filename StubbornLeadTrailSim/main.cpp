...
int main(int argc, const char *argv[]) {
    ...
    for(double hashVal = 0.005; hashVal < .51; hashVal+=.005) {
        ...
        auto forkStrat1 = createStubbornLeadTrailStrategy(NOISE_IN_TRANSACTIONS, 1);


        MinerParameters stubbornLeadTrailMinerParams = {0, std::to_string(0), attackerPower, NETWORK_DELAY, COST_PER_SEC_TO_MINE};
        MinerParameters defaultinerParams = {2, std::to_string(1), honestPower, NETWORK_DELAY, COST_PER_SEC_TO_MINE};

        
        miners.push_back(std::make_unique<Miner>(stubbornLeadTrailMinerParams, *forkStrat1));
        ...
        GAMEINFO("Gamma" << "Fraction earned by stubborn lead trail:" << fractionOfProfits << " with " << attackerPower << " fraction of hash power" << "Fraction by honest: " << honestFractionOfProfits << std::endl);
