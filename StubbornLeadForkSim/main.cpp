...
int main(int argc, const char *argv[]) {
    ...
    //#########################################################################################
    //idea of simulation: 2 miners, only an honest, and a stubborn lead fork miner. Run many games, with the
    //size of the two changing. Plot the expected profit vs. actual profit.
    GAMEINFO("#####\nRunning Stubborn Lead Fork Mining Simulation\n#####" << std::endl);
    std::ofstream plot;
    ...
    plot << "Gamma, Stubborn Lead Fork Profit, Stubborn Lead Fork Hash Rate, Honest Profit" << std::endl;
    //start running games
    ...
    for(double hashVal = 0.005; hashVal < .51; hashVal+=.005) {
        ...
        auto forkStrat1 = createStubbornLeadForkStrategy(NOISE_IN_TRANSACTIONS);


        MinerParameters stubbornForkMinerParams = {0, std::to_string(0), attackerPower, NETWORK_DELAY, COST_PER_SEC_TO_MINE};
        MinerParameters defaultinerParams = {2, std::to_string(1), honestPower, NETWORK_DELAY, COST_PER_SEC_TO_MINE};

        
        miners.push_back(std::make_unique<Miner>(stubbornForkMinerParams, *forkStrat1));
